...
Given a BLAST output, filters query-subject pairs such that only HSPs
with a coverage greater than `mncvrg` (specified in the pipeline
parameters) remain. Filters both: query-subject and subject-query pairs,
if one of the coverages is insufficient. HSP coverage is obtained from
the BLAST column `qcovs`.
